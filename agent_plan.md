...
**Status: OBSERVATION RULES LIVE — enforcement review and Strix scan pending.**
...
- Docker and Strix 1.5.3 are installed. Exploit validation did not start because
  ...
  reviewed.
- Published observation-only Vercel rules on both production projects. Web logs
  common exploit probes and auth bursts above 20 requests per minute by IP;
  Studio logs the same probes and AI bursts above 20 requests per minute. The
  exceeded action is Log, not Deny/429, so no reader or editor is blocked during
  baseline collection. Both canonical entry points still return HTTP 200.
- The current account plan refused a second rate-limit rule per project with a
  provider 401. Application rate limiters remain authoritative for public writes
  and Studio Server Actions; broader firewall enforcement needs a plan change or
  provider allowance plus the documented false-positive review.

## 41. KUR-103 — canonical deployment handoff cleanup (2026-09-01)

**Status: DEPLOYED — CI and cross-host routing verified.**

- Re-audited every checked-in Vercel hostname and both live stable aliases.
  Public Web returns the expected locale redirect from
  `kurasikapa-web.vercel.app`; Studio sign-in returns HTTP 200 from
  `kurasikapa-studio.vercel.app/studio/en/sign-in`.
- Corrected the remaining Web `/studio/*` rewrite that still named Vercel's
  long autogenerated Studio project host. Cross-app routing now consistently
  uses the deliberate Studio alias already used by environment handoff,
  invitations, cron and deployment documentation.
- Commit `8dbb34b` is at exact `origin/main` parity. CI run `33482001878` is
  green across Quality gates, Go service, secret scan and Lighthouse. Web
  production deployment `dpl_BQX2AvuiyJ6eCa7khBJjtn3VeBEF` is Ready and owns
  `kurasikapa-web.vercel.app`; Studio deployment
  `dpl_75Z6hTuo6x1gw36FWMvHRtLycFnZ` is Ready and owns
  `kurasikapa-studio.vercel.app`.
