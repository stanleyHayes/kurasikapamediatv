import { processNarrationsViaApi } from '@kurasikapa/web-kit/bff/article-narration'
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (apiUrl === undefined || secret === undefined) {
    return Response.json({ error: 'Narration processing is not configured' }, { status: 503 })
  }

  const result = await processNarrationsViaApi({ baseUrl: apiUrl, cronSecret: secret })
