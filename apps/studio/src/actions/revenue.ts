...
import { activateAdCampaign, approveAdvertiserProposal, createAndActivateAdCampaign, createAndActivateAffiliateLink, createAndActivateMembershipPlan, createAndActivateProduct, rejectAdvertiserProposal, publishClassified, updateAdCampaign } from '@kurasikapa/web-kit/bff/revenue'
...
const campaignRefSchema = z.object({ id: z.string().trim().min(1).max(100) })

/**
 * Corrects a campaign's terms without touching whether it is live.
 *
 * Campaigns used to be create-and-activate only, so a placeholder budget was
 * permanent and the only remedy was editing the database by hand.
 */
export async function updateAdCampaignAction(id: unknown, input: unknown): Promise<ActionResult<undefined>> {
  return attempt(async () => {
    const parsed = parseInput(adSchema, input)
    await updateAdCampaign(await requireActor(), campaignRefSchema.parse({ id }).id, {
      name: parsed.name, advertiser: parsed.advertiser, locale: parsed.locale, slot: parsed.slot,
      creativeURL: parsed.creativeURL, altText: parsed.altText, landingURL: parsed.landingURL,
      budget: { minor: parsed.budgetMinor, currency: parsed.currency }, cpmMinor: parsed.cpmMinor,
      priority: parsed.priority, startsAt: parsed.startsAt, endsAt: parsed.endsAt,
...
/** Puts a campaign live. Deliberately separate from saving its terms. */
export async function activateAdCampaignAction(input: unknown): Promise<ActionResult<undefined>> {
  return attempt(async () => {
    await activateAdCampaign(await requireActor(), campaignRefSchema.parse(input).id)

    return undefined
  })
}

const proposalReviewSchema = z.object({ id: z.string().trim().min(1).max(100) })
const proposalRejectionSchema = proposalReviewSchema.extend({ note: z.string().trim().min(10).max(500) })

export async function approveAdvertiserProposalAction(input: unknown): Promise<ActionResult<{ id: string }>> {
  return attempt(async () => {
    const parsed = parseInput(proposalReviewSchema, input)
    const result = await approveAdvertiserProposal(await requireActor(), parsed.id)
    return { id: result.id }
  })
}

export async function rejectAdvertiserProposalAction(input: unknown): Promise<ActionResult<{ id: string }>> {
  return attempt(async () => {
    const parsed = parseInput(proposalRejectionSchema, input)
    const result = await rejectAdvertiserProposal(await requireActor(), parsed.id, parsed.note)
    return { id: result.id }
  })
}
...
const affiliateSchema = z.object({ partner: z.string().trim().min(2).max(100), title: z.string().trim().min(3).max(140), category: z.string().trim().min(2).max(60), description: z.string().trim().min(20).max(1000), disclosure: z.string().trim().min(10).max(240), imageURL: z.url().startsWith('https://'), imageAlt: z.string().trim().min(5).max(180), destinationURL: z.url().startsWith('https://'), commissionNote: z.string().trim().max(180) })
export async function createAffiliateLinkAction(input: unknown): Promise<ActionResult<{ id: string }>> { return attempt(async () => createAndActivateAffiliateLink(await requireActor(), parseInput(affiliateSchema, input))) }
