...
export function LiveControlRoom({ locale, current, history, actions = serverActions }: { locale: string; current: CurrentBroadcast | null; history: readonly BroadcastRow[]; actions?: LiveControlActions }): React.ReactElement {
  ...
  const start = (formData: FormData): void => {
    startTransition(async () => {
      ...
      const result = await actions.start({ title, locale })
...
function SafetyPanel(): React.ReactElement { return <aside className="border-t-8 border-secondary bg-surface-container p-6"><p className="broadcast-kicker text-primary">Before air</p><ol className="mt-5 space-y-4 text-sm text-on-surface-variant"><li><strong className="text-on-surface">01.</strong> Confirm microphone, camera and programme title.</li><li><strong className="text-on-surface">02.</strong> Use OBS streaming service Custom and paste both values.</li><li><strong className="text-on-surface">03.</strong> Start streaming in OBS, then open the public Live page to monitor.</li><li><strong className="text-on-surface">04.</strong> End here after the programme to tear down the billable channel.</li></ol></aside> }
...
function BroadcastHistory({ rows, locale }: { rows: readonly BroadcastRow[]; locale: string }): React.ReactElement {
  return <section className="border border-outline-variant bg-surface-container-lowest p-6 xl:col-span-2"><div className="flex items-end justify-between gap-4"><div><p className="broadcast-kicker text-primary">Transmission ledger</p><h2 className="mt-1 font-display text-2xl font-bold">Recent broadcasts</h2></div><span className="text-xs text-on-surface-variant">Latest 10</span></div>{rows.length === 0 ? <p className="mt-6 border-l-4 border-outline px-4 py-3 text-sm text-on-surface-variant">No broadcasts have been provisioned in this language.</p> : <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[38rem] text-left text-sm"><thead className="border-b border-outline-variant text-[10px] uppercase tracking-[.14em] text-on-surface-variant"><tr><th scope="col" className="py-3">Programme</th><th scope="col">Status</th><th scope="col">Started</th><th scope="col">Ended</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-b border-outline-variant/60"><td className="py-4 pr-4 font-semibold">{row.title}</td><td><span className="border border-outline px-2 py-1 text-[10px] font-bold uppercase">{row.state}</span></td><td>{formatDate(row.startedAt, locale)}</td><td>{formatDate(row.endedAt, locale)}</td></tr>)}</tbody></table></div>}</section>
}
