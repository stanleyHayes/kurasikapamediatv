...
const KEYS: readonly SitePageKey[] = ['about', 'team', 'contact', 'careers', 'faq', 'advertise', 'privacy', 'terms', 'cookies']
...
export function SitePageEditor({ pages }: { pages: readonly EditableSitePage[] }): React.ReactElement {
  const [key, setKey] = useState<SitePageKey>('about')
...
function Editor({ initial }: { initial: EditableSitePage }): React.ReactElement {
  ...
  return <section className="border border-outline-variant bg-surface-container-lowest"><header className="border-b border-outline-variant p-5"><p className="broadcast-kicker text-primary">{initial.locale} · {initial.key}</p><h2 className="mt-2 font-display text-2xl font-semibold text-on-surface">Page content</h2><p className="mt-2 text-sm text-on-surface-variant">Use Markdown headings, paragraphs and lists. Saving updates the public page for this language.</p></header><div className="space-y-5 p-5 md:p-7"><Field label="Page title"><input value={title} onChange={(event) => { setTitle(event.target.value) }} className="h-12 w-full border border-outline-variant bg-surface px-4 text-on-surface outline-none focus:border-primary" /></Field><Field label="Standfirst"><textarea value={lead} onChange={(event) => { setLead(event.target.value) }} rows={3} className="w-full border border-outline-variant bg-surface p-4 text-on-surface outline-none focus:border-primary" /></Field><Field label="Body — Markdown"><textarea value={body} onChange={(event) => { setBody(event.target.value) }} rows={18} className="w-full border border-outline-variant bg-surface p-4 font-mono text-sm leading-6 text-on-surface outline-none focus:border-primary" /></Field><div className="flex flex-wrap items-center gap-4"><button type="button" disabled={pending || title.trim() === '' || body.trim() === ''} onClick={save} className="bg-primary px-6 py-3 text-sm font-bold uppercase text-on-primary disabled:opacity-40">{pending ? 'Publishing…' : 'Publish page'}</button>{message && <p role="status" className="text-sm text-on-surface-variant">{message}</p>}</div></div></section>
}
