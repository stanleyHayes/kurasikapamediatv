...
async function main(): Promise<void> {
  ...
  await seedManagedContent(db, { demoSeed: DEMO_SEED, now: at(0), inDays })
  ...
  console.error(
    `seeded ${String(published)} published, 1 draft, 1 in review, 1 scheduled, ` +
      ...
      `${String(MANAGED_PAGE_COUNT)} managed pages, ${String(PRESENTER_COUNT)} presenters, ` +
      `${String(PROGRAMME_COUNT)} programmes and ${String(SCHEDULE_SLOT_COUNT)} schedule slots`,
  )
