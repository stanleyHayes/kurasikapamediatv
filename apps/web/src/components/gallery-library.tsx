...
function Empty({ locale }: { locale: string }): React.ReactElement {
  ...
  return <section className="signal-grid border-y-2 border-on-surface bg-surface-container-low px-5 py-20 text-center"><span aria-hidden className="mx-auto grid size-16 animate-pulse place-items-center bg-primary text-3xl text-on-primary">▧</span><h2 className="mx-auto mt-6 max-w-2xl font-display text-4xl font-bold tracking-tight">{french ? 'Le desk visuel prépare son premier reportage.' : 'The visual desk is preparing its first report.'}</h2><p className="mx-auto mt-4 max-w-xl leading-7 text-on-surface-variant">{french ? 'Des reportages photo originaux et des vidéos sous-titrées paraîtront ici avec contexte et crédits.' : 'Original photo stories and captioned video reports will appear here with context and complete credits.'}</p><div className="mx-auto mt-9 grid max-w-3xl gap-3 sm:grid-cols-3"><Signal text="Original reporting"/><Signal text="Complete context"/><Signal text="Accessible video"/></div></section>
}
function Signal({ text }: { text: string }): React.ReactElement { return <div className="border border-outline-variant bg-surface-container-lowest p-4 text-sm font-bold">{text}</div> }
