...
function ScheduleCard({ item, locale, reminder, liveLabel, recordedLabel }: { item: GuideSlot; locale: string; reminder: string; liveLabel: string; recordedLabel: string }): React.ReactElement {
  ...
  const href = calendarDataUrl({ id: item.id, title: item.programme.title, description: 'Watch on Kurasikapa Media TV.', startsAt: start, endsAt: new Date(item.endsAt), location: `https://kurasikapamediatv.com/${locale}/live` })
