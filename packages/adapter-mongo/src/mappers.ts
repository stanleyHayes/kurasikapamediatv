...
export const articleToDomain = (doc: ArticleDocument): Article =>
  Article.reconstitute({
    ...
    narration: doc.narration === undefined ? null : {
      ...doc.narration,
      assetId: assetId(doc.narration.assetId),
      sourceRevisionId: revisionId(doc.narration.sourceRevisionId),
    },
    status: doc.status,
...
export const articleToDocument = (article: Article, updatedAt: Date): ArticleDocument => {
  ...
  return {
    ...
    ...(props.narration === undefined || props.narration === null ? {} : { narration: props.narration }),
    status: props.status,
